...
const MainContent: React.FC<MainContentProps> = (props) => {
  ...
  const handleGenSeo = async () => {
    ...
    setIsLoading(true);
    try {
      const combinedDetails = `Product Name: ${productName}\n\nUser Details:\n${imageDetails}`;
      const res = await generateSeoContent(props.userApiKey, props.ficheProduitImageBase64, props.ficheProduitImageMimeType!, combinedDetails);
      setSeoResult(res);
    ...
    <div className="w-3/5 bg-[#181a1f] p-8 rounded-[24px] shadow-2xl flex flex-col overflow-y-auto border border-gray-800 scrollbar-thin scrollbar-thumb-gray-700 scrollbar-track-transparent">
      ...
      {props.activeNav === 'Product Listing' && (
        <div className="animate-in fade-in duration-500">
          ...
          <div className="mb-10">
            <h3 className="text-sm font-bold mb-4 text-violet-400 uppercase tracking-widest flex items-center">
              <span className="mr-3 p-2 bg-violet-500/10 rounded-lg">✍️</span> Détails supplémentaires
            </h3>
            <textarea 
              value={imageDetails} 
              onChange={e => setImageDetails(e.target.value)} 
              placeholder="Ajoutez ici tous les détails importants sur votre produit : matériaux, dimensions, couleurs disponibles, histoire du produit, etc..." 
              className="w-full p-5 bg-gray-800/50 border border-gray-700 rounded-2xl focus:ring-2 focus:ring-violet-500 outline-none transition-all h-40 text-sm text-white resize-none" 
            />
          </div>
          ...
          {seoResult && (
            <div className="space-y-10 pb-20">
              <div className="bg-gray-800/60 p-8 rounded-[32px] border border-gray-700 shadow-2xl group relative">
                <div className="flex justify-between items-center mb-6">
                  <h4 className="text-violet-400 font-black uppercase tracking-[0.2em] text-[10px]">🏆 Choisissez Votre Titre (5 Options SEO)</h4>
                ...
                <div className="space-y-3 mb-6">
                  {seoResult.titleOptions && seoResult.titleOptions.map((titleOption, idx) => (
                    <button
                      key={idx}
                      onClick={() => setEditableTitle(titleOption)}
                      className={`w-full text-left p-4 rounded-xl border-2 transition-all ${
                        editableTitle === titleOption 
                          ? 'border-violet-500 bg-violet-500/10 shadow-lg' 
                          : 'border-gray-700 bg-gray-900/30 hover:border-violet-400 hover:bg-gray-800/50'
                      }`}
                    >
                      <div className="flex items-start">
                        <span className={`font-black text-xs mr-3 mt-1 ${editableTitle === titleOption ? 'text-violet-400' : 'text-gray-500'}`}>
                          {idx + 1}
                        </span>
                        <span className={`text-sm font-bold ${editableTitle === titleOption ? 'text-white' : 'text-gray-300'}`}>
                          {titleOption}
                        </span>
                      </div>
                    </button>
                ...
                <div className="border-t border-gray-700 pt-4">
                  <p className="text-[9px] text-gray-500 uppercase tracking-widest mb-2 font-bold">Titre Sélectionné (Éditable) :</p>
                  <input 
                    value={editableTitle} 
                    onChange={(e) => setEditableTitle(e.target.value)}
                    className="w-full bg-gray-900/50 text-lg font-bold text-white outline-none p-4 border border-gray-700 rounded-xl focus:ring-2 focus:ring-violet-500"
                  />
                </div>
              </div>
